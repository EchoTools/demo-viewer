	...
	*/
	...
	public void RenderFrame(Frame viewingFrame, Frame previousFrame)
	{
		...
		if (hasBonePlayers)
		{
			...
			{
				Debug.Log($"[DemoStart] Bone data found: {viewingFrame.bones.user_bones.Length} players with bones");
				foreach (BonePlayer bp in viewingFrame.bones.user_bones)
				{
					if (bp != null)
					{
						Debug.Log($"[DemoStart]   Player ID {bp.playerid}: bone_o={bp.bone_o?.Length ?? 0}, bone_t={bp.bone_t?.Length ?? 0}");
					}
				}
			}

			// Create a dictionary of bones by playerid for easy lookup
			Dictionary<int, BonePlayer> bonesByPlayerid = new Dictionary<int, BonePlayer>();
			foreach (BonePlayer bonePlayer in viewingFrame.bones.user_bones)
			{
				if (bonePlayer != null)
				{
					bonesByPlayerid[bonePlayer.playerid] = bonePlayer;
				}
			}
			...
			for (int i = 0; i < playerList.Count; i++)
			{
				...
				if (!playerV4Objects.ContainsKey(player.name))
				{
					...
					{
						Debug.LogError("[DemoStart] playerV4Prefab is not assigned! Cannot render bones.");
						break;
					}
